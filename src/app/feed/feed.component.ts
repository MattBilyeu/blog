...
export class FeedComponent implements OnInit {
  ...
  ngOnInit() {
    this.contentfulService.getEntries().then(entries => {
  ...
  generateRssXml(): SafeHtml {
    ...
    for (let i = 0; i < this.entries.length; i++) {
      ...
      const item = `<item><title>${title}</title><link>${link}</link><description>${description}</description></item>`;
      rssContentMain = rssContentMain.concat(item);
    }
    const closing = '</channel></rss>';
    const combined = rssContentPreamble.concat(rssContentMain).concat(closing);
